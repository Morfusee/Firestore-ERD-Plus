...
beforeEach(async () => {
  const owner = await request(app).post("/users").send({
    username: "owner_user",
  ...
  // Set usernames
  ownerUsername = owner.body.data.createdUser.username;
  user1Username = user1.body.data.createdUser.username;
  user2Username = user2.body.data.createdUser.username;
...
describe("Members Tests", () => {
  ...
  it("should add a new member to the project", async () => {
    const res = await request(app)
      ...
      .send({ username: user2Username, role: "Viewer" });
    ...
    expect(
      updatedUser?.sharedProjects.map((item) => item.toString())
    ).toContain(projectId);
  });
  ...
  it("should return 409 if user is already a member", async () => {
    await request(app)
      .post(`/projects/${projectId}/members`)
      .send({ username: user1Username, role: "Viewer" });
  ...
  it("should return 404 if user does not exist", async () => {
    const dummyUsername = "dummy_username";
    const res = await request(app)
      .post(`/projects/${projectId}/members`)
      .send({ username: dummyUsername, role: "Viewer" });
  ...
  it("should update a member's role", async () => {
    ...
    expect(res.body.data.updatedMember.role).toBe("Editor");

    const updatedProject = await Project.findById(projectId);
    expect(
      updatedProject!.members.find((m) => m.userId.equals(user1Id))!.role
    ).toBe("Editor");
  });
  ...
  it("should remove a member from the project", async () => {
    ...
    const res = await request(app).delete(
      `/projects/${projectId}/members/${user2Id}`
    );
  ...
  it("should not allow removing the owner", async () => {
    const res = await request(app).delete(
      `/projects/${projectId}/members/${ownerId}`
    );
