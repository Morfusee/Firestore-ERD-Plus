...
const ReactiveThemeContext = createContext<ReactiveThemeContextValue | undefined>(undefined);

export function useReactiveTheme() {
  const ctx = useContext(ReactiveThemeContext);
  if (!ctx)
    throw new Error("useReactiveTheme must be used within a ReactiveThemeProvider");
  return ctx;
}

export function ReactiveThemeProvider({ children }: { children: React.ReactNode }) {
