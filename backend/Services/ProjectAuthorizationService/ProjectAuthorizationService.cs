...
public class ProjectAuthorizationService(MongoDbContext context) : IProjectAuthorizationService
{
    ...
    private static bool Allows(MemberRole role, ProjectPermission permission) => permission switch
    {
        ProjectPermission.Read => true,
        ProjectPermission.Write => role is MemberRole.Editor or MemberRole.Admin or MemberRole.Owner,
    ...
    public async Task<bool> MatchesUserEmailAsync(string userId, string email)
    {
        ...
        var user = await _context.Users
            .Find(u => u.Id == userId && u.Email.Equals(email, StringComparison.CurrentCultureIgnoreCase))
            .FirstOrDefaultAsync();

        return user != null;
    }

    public async Task<bool> CanAccessProjectAsync(string projectId, string userId, ProjectPermission permission)
    {
    ...
    public async Task<bool> CanAccessVersionAsync(string versionId, string userId, ProjectPermission permission)
    {
    ...
    public async Task<bool> CanAccessHistoryAsync(string historyId, string userId, ProjectPermission permission)
    {
    ...
    public FilterDefinition<Project> GetAccessibleProjectsFilter(string userId)
    {
        ...
        {
            return Builders<Project>.Filter.Eq(p => p.GeneralAccess.AccessType, GeneralAccessType.Link);
        }
